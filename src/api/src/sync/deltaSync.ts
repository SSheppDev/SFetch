...
async function loadEnabledFields(
  ...
  return fieldResult.rows
    .filter((row) =>
      !SF_SYSTEM_FIELDS.has(row.field_api_name) &&
      !SF_COMPOUND_TYPES.has(row.sf_type.toLowerCase()) &&
      !(row.field_api_name === 'LastModifiedDate' && !hasSystemModstamp)
    )
    .map((row) => ({
      sfName: row.field_api_name,
      pgName: row.field_api_name.toLowerCase(),
      pgType: row.pg_type,
    }))
}

function buildDeltaSoql(
  objectApiName: string,
  ...
  const selectFields = [...systemSelect, ...fields.map((field) => field.sfName)].join(', ')
...
function missingFieldFromBulkError(message: string): string | null {
  const match = message.match(/No such column '([^']+)'/i)
  return match?.[1] ?? null
}
...
export async function runDeltaSync(orgId: string, targetObject?: string): Promise<void> {
  ...
  for (const obj of configResult.rows) {
    ...
    try {
      ...
      while (true) {
        fields = await loadEnabledFields(orgId, objectApiName, hasSystemModstamp)
        const soql = buildDeltaSoql(
          objectApiName,
          lastDeltaSync,
          trackingField,
          auditFields.hasCreatedDate,
          fields
        )

        try {
          queryResult = await bulkClient.query(soql, estimatedRows)
          break
        } catch (err) {
          const error = err instanceof Error ? err : new Error(String(err))
          const message = error.message ?? ''
          const missingModstamp =
            message.includes("No such column 'SystemModstamp'") ||
            message.includes("No such column 'CreatedDate'")

          if (missingModstamp) {
            console.warn(
              `[delta-sync] [${orgId}] ${objectApiName}: object lacks standard audit fields — switching to LastModifiedDate`
            )
            await pool.query(
          ...
          const missingField = missingFieldFromBulkError(message)
          if (!missingField) {
            throw error
          }

          const disableResult = await pool.query(
            `UPDATE sfdb.field_config
             SET enabled = false
             WHERE org_id = $1
               AND object_api_name = $2
               AND field_api_name = $3
               AND enabled = true`,
            [orgId, objectApiName, missingField]
          )

          if ((disableResult.rowCount ?? 0) === 0) {
            throw error
          }
          ...
          if (attempt > 100) {
            throw new Error(
              `[delta-sync] [${orgId}] ${objectApiName}: exceeded missing-field retry limit`
            )
          }

          console.warn(
            `[delta-sync] [${orgId}] ${objectApiName}: disabling unavailable field ${missingField} and retrying`
          )
        }
      }

      if (!queryResult) {
        const fallbackFields = (await loadEnabledFields(orgId, objectApiName, false))
          .filter((field) => field.sfName !== 'LastModifiedDate')
        const fallbackSoql = buildDeltaSoql(
          objectApiName,
          lastDeltaSync,
          'LastModifiedDate',
          false,
        ...
        queryResult = await bulkClient.query(fallbackSoql, estimatedRows)
      }
